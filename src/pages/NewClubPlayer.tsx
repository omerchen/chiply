...
function NewClubPlayer() {
  ...
  return (
    <Container maxWidth="sm" sx={{ mt: 3, mb: 3 }}>
      <ClubBreadcrumbs clubId={clubId!} clubName={clubName} currentPage="Invite Player" />
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2 }}>
        ...
        {showNewPlayerForm && (
          <Box>
            ...
            >
              {loading ? <CircularProgress size={24} sx={{ color: 'white' }} /> : 'Add Player'}
            </Button>
          </Box>
        )}

        <Dialog open={showConfirmDialog} onClose={() => !loading && setShowConfirmDialog(false)}>
          <DialogTitle>Player Found</DialogTitle>
          <DialogContent>
            {foundPlayer && (
              <Typography>
                {foundPlayer.firstName} {foundPlayer.lastName} ({foundPlayer.email})
              </Typography>
            )}
            <Typography sx={{ mt: 2 }}>
